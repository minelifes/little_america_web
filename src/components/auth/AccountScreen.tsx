...
// AuthContext.open()).
//
// Avatar/name/email are real (from AuthContext.userDisplay). The bonus
...
export default function AccountScreen() {
  ...
  const email = auth.userDisplay?.email ?? "";
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <Box sx={{ flex: 1, overflowY: "auto", px: 3, display: "flex", flexDirection: "column", gap: 3 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box sx={{ minWidth: 0 }}>
            {name && (
              <Box sx={{ fontSize: 15, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis" }}>{name}</Box>
            )}
            <Box sx={{ fontSize: 13, color: colors.additionalTextColor, overflow: "hidden", textOverflow: "ellipsis" }}>
              {email}
            </Box>
          </Box>
        </Box>
        ...
        >
          {BONUS_DECORATIONS.map((d, i) => (
            ...
            >
              <Box component="span" sx={{ fontSize: d.size * 0.42, fontWeight: 700, color: "#ffffff" }}>
                Б
              </Box>
            </Box>
          ))}
          ...
          >
            <Box component="span" sx={{ fontSize: 13, color: "rgba(0,0,0,0.35)" }}>
              Баланс бонусів:
            </Box>
            <Box
              sx={{
            ...
            >
              ...
              <Box component="span" sx={{ fontSize: 18, fontWeight: 700, color: ACCENT_PINK_STRONG }}>
                {wallet?.balance ?? 0}
              </Box>
            </Box>
          </Box>
      ...
      <Box sx={{ flexShrink: 0, p: 3, display: "flex", justifyContent: "center" }}>
        <Box
          component="button"
          type="button"
          onClick={auth.logout}
